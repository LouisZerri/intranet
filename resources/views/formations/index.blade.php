                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex space-x-3">
                        ...
                        @if (Auth::user()->isManager() || Auth::user()->isAdministrateur())
                            <a href="{{ route('formations.manage') }}"
                                class="inline-flex items-center px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-all duration-200">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                Gérer les demandes
                                @php
                                    $pendingCount = \App\Models\FormationRequest::forUser(Auth::user())
                                        ->pending()
                                        ->count();
                                @endphp
                                @if ($pendingCount > 0)
                                    <span class="ml-2 px-2 py-0.5 text-xs bg-white text-green-700 rounded-full font-bold">
                                        {{ $pendingCount }}
                                    </span>
                                @endif
                            </a>
                        ...
                        <div class="flex items-center">
                            ...
                            <div class="ml-3">
                                ...
                                <div class="text-xl font-semibold text-blue-900">
                                    {{ Auth::user()->getFormationHoursThisYear() }}h</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-green-50 overflow-hidden rounded-lg p-4 border border-green-200">
                        <div class="flex items-center">
                            ...
                            <div class="ml-3">
                                ...
                                <div class="text-xl font-semibold text-green-900">{{ $userStats['completed_formations'] }}
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-yellow-50 overflow-hidden rounded-lg p-4 border border-yellow-200">
                        <div class="flex items-center">
                            ...
                            <div class="ml-3">
                                ...
                                <div class="text-xl font-semibold text-yellow-900">{{ $userStats['pending_requests'] }}
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="bg-white shadow rounded-lg p-6">
            <form method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <div class="flex items-end space-x-2">
                    <button type="submit"
                        class="flex-1 inline-flex items-center justify-center px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @if (request()->hasAny(['search', 'category', 'level', 'format']))
                        <a href="{{ route('formations.index') }}"
                            class="inline-flex items-center px-3 py-2 bg-gray-300 text-gray-700 text-sm font-medium rounded-md hover:bg-gray-400">
                ...
                @if ($formations->count() > 0)
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach ($formations as $formation)
                            <div
                                class="bg-white border border-gray-200 rounded-lg hover:shadow-lg transition-all duration-200 overflow-hidden group">
                                <div class="p-6">
                                    <!-- Header avec niveau et format -->
                                    <div class="flex items-center justify-between mb-4">
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-{{ $formation->level === 'debutant' ? 'green' : ($formation->level === 'intermediaire' ? 'yellow' : 'red') }}-100 text-{{ $formation->level === 'debutant' ? 'green' : ($formation->level === 'intermediaire' ? 'yellow' : 'red') }}-800">
                                    ...
                                    <div class="mb-4">
                                        <h3
                                            ...
                                            <a
                                                href="{{ route('formations.show', $formation) }}">{{ $formation->title }}</a>
                                        </h3>
                                        <p class="text-sm text-gray-600 mt-2 line-clamp-3">
                                            {{ Str::limit($formation->description, 120) }}</p>
                                    </div>
