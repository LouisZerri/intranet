...
@section('title', isset($quote) ? 'Modifier le devis' : 'Nouveau devis')
...
@section('content')
        ...
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">
                    {{ isset($quote) ? '✏️ Modifier le devis' : '➕ Nouveau devis' }}
                </h1>
                <p class="mt-1 text-sm text-gray-500">
                    {{ isset($quote) ? 'Devis ' . $quote->quote_number : 'Créez un devis pour votre client' }}
                </p>
            </div>
            <a href="{{ isset($quote) ? route('quotes.show', $quote) : route('quotes.index') }}" 
               class="inline-flex items-center px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium rounded-lg">
                ← Retour
            </a>
        </div>

        <form method="POST" action="{{ isset($quote) ? route('quotes.update', $quote) : route('quotes.store') }}" id="quoteForm">
            @csrf
            @if(isset($quote))
                @method('PUT')
            @endif
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select name="service" id="service" required
                                    ...
                                    @foreach($services as $key => $label)
                                        <option value="{{ $key }}" {{ old('service', $quote->service ?? '') === $key ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="date" name="validity_date" id="validity_date"
                                    value="{{ old('validity_date', isset($quote) && $quote->validity_date ? $quote->validity_date->format('Y-m-d') : '') }}"
                    ...
                    <div class="bg-gradient-to-r from-indigo-50 to-blue-50 border-2 border-indigo-200 rounded-lg p-6">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <button type="button" 
                                    onclick="document.getElementById('predefined-services-list').classList.toggle('hidden')"
                        ...
                        <p class="text-sm text-indigo-700 mb-4">
                            Ajoutez rapidement des prestations types avec leurs tarifs prédéfinis
                        </p>
                ...
                <div class="lg:col-span-1 space-y-4">
                    ...
                    <div class="bg-white shadow rounded-lg p-6">
                        ...
                        <div class="space-y-3">
                            <button type="submit" 
                                class="w-full inline-flex justify-center items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                                </svg>
                                {{ isset($quote) ? 'Enregistrer les modifications' : 'Créer le devis' }}
                            </button>
                            <a href="{{ isset($quote) ? route('quotes.show', $quote) : route('quotes.index') }}" 
                               class="w-full inline-flex justify-center items-center px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium rounded-lg">
                                Annuler
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                        ...
                        <ul class="space-y-2 text-sm text-blue-800">
                            <li>• Cliquez sur une prestation prédéfinie pour l'ajouter</li>
    ...
    <template id="line-template">
        <div class="line-item border border-gray-300 rounded-lg p-4 bg-gray-50">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            @if(isset($quote) && $quote->items->count() > 0)
