...
@section('content')
    <div class="space-y-6">
        ...
        {{-- Résumé --}}
        <div class="bg-gradient-to-r from-blue-500 to-indigo-600 rounded-lg shadow-lg p-6">
            <h2 class="text-lg font-semibold mb-4 text-white">💰 Résumé des revenus</h2>
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div class="bg-white rounded-lg p-4">
                    <div class="text-sm text-gray-600">Factures payées</div>
                    <div class="text-3xl font-bold mt-1 text-gray-600">{{ $data['invoice_count'] }}</div>
                </div>
                <div class="bg-white rounded-lg p-4">
                    <div class="text-sm text-gray-600">Total HT</div>
                    <div class="text-3xl font-bold mt-1 text-gray-600">{{ number_format($data['total_ht'], 0, ',', ' ') }} €
                    </div>
                </div>
                <div class="bg-white rounded-lg p-4">
                    <div class="text-sm text-gray-600">TVA collectée</div>
                    <div class="text-3xl font-bold mt-1 text-gray-600">{{ number_format($data['total_tva'], 0, ',', ' ') }}
                        €</div>
                </div>
                <div class="bg-white rounded-lg p-4">
                    <div class="text-sm text-gray-600">Total TTC</div>
                    <div class="text-3xl font-bold mt-1 text-gray-600">{{ number_format($data['total_ttc'], 0, ',', ' ') }}
                        €</div>
                </div>
            </div>
        </div>
        ...
        @if (count($data['invoices']) > 0)
            <div class="bg-white shadow rounded-lg overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h3 class="text-lg font-semibold text-gray-900">📋 Détail des factures ({{ count($data['invoices']) }})
                    </h3>
                </div>
